...
(async () => {
  try {
    ...
    const upload = await request({ hostname: '127.0.0.1', port: 7001, path: '/api/import/upload', method: 'POST', headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'multipart/form-data; boundary=' + boundary, 'Content-Length': body.length } }, body);
